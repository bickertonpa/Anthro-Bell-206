...
library(ggrepel) #for spacing labels.
...
write.csv(df_anthros, "output/df_anthros.csv", row.names = FALSE)
...
- Raw values (e.g., in millimeters, or kg), useful for direct dimensional interpretation and cockpit design reference, and
...
anthro_summary <- rbind(anthro_summary_f,anthro_summary_m) %>% dplyr::select(-Name)
write.csv(anthro_summary, "output/anthro_summary.csv", row.names = FALSE)
datatable(anthro_summary,height = 800)
```
...
# Combine Study and Population Datasets
f_sex <- clean_f %>%
  mutate(
    Sex = "F",
    Group = "Study Female"
...
ggplot(clean_combined, aes(x = `Combo Leg`, y = `Sitting Height`, color = Group)) +
  ...
  geom_point(data = study_df, aes(x = `Combo Leg`, y = `Sitting Height`),
             inherit.aes = FALSE, shape = 2, size = 3, stroke = 1.1) +
  # Label only the Study data with plain text (no box). Use geom_label for text + box
  geom_label(
...
pca_data_no_rotation <- principal(scaled_data,nfactors=6,covar=TRUE,rotate="none")

loadings_no_rot <- pca_data_no_rotation$loadings %>% as.data.frame()
loadings_no_rot <- loadings_no_rot[1:(nrow(loadings_no_rot)/6),]
print(pca_data_no_rotation$loadings)

ggplot(loadings_no_rot, aes(x = PC1, y = PC2))+geom_point()+
  scale_x_continuous(limits = c(0,1)) +  # Define x-axis limits
  ...
  #geom_text(aes(label = rownames(loadings_no_rot)), vjust = 1.5, hjust = -0.5, size = 4)   # Text labels with row 
  geom_text_repel(aes(label = rownames(loadings_no_rot)),point.padding = 0.1,min.segment.length = 0.1,size = 3)   # Text labels with row 
...
ggplot(loadings_varimax, aes(x = RC1, y = RC2))+geom_point()+
  ...
  geom_text_repel(aes(label = rownames(loadings_varimax)),point.padding = 0.1,min.segment.length = 0.1,size = 3)   # Text labels with row 
...
write.csv(df_onv, "output/df_onv.csv", row.names = FALSE)
write.csv(df_onv_737, "output/df_onv_737.csv", row.names = FALSE)
write.csv(df_head_clear, "output/df_head_clear.csv", row.names = FALSE)
write.csv(df_shin, "output/df_shin.csv", row.names = FALSE)
write.csv(df_motion, "output/df_motion.csv", row.names = FALSE)
write.csv(df_rudder, "output/df_rudder.csv", row.names = FALSE)
write.csv(df_hand_reach, "output/df_hand_reach.csv", row.names = FALSE)
write.csv(df_controls, "output/df_controls.csv", row.names = FALSE)
...
write.csv(df_seat_positions, "output/df_seat_positions.csv", row.names = FALSE)
