...
To further explore the multivariate structure of anthropometric variability relevant to cockpit fit, this section applies Principal Component Analysis (PCA) using the six most critical cockpit-relevant body dimensions—commonly mentioned earlier.  PCA combines the variables in a way that maximizes the variance of the PC1 as evident by the higher Sums of Squares of the `loadings`, which is good for dimension reduction but bad for interpretability. If we study the absolute values of the weights for PC1, we see that all values of PC1 are ~0.5. The same is true of PC2. By using the absolute values, it is impossible to tell which variables are associated to PC1 or PC2.
...
ggplot(weights_no_rotation, aes(x = PC1, y = PC2))+geom_point()+
  ...
  geom_text(aes(label = rownames(weights_no_rotation)), vjust = 1.5, hjust = -0.5, size = 3)   # Text labels with row 
...
Principal Component 1 and 2 of the core six measures accounts for ~85% of the variability of the underlying dataset and is sufficient for accommodation modelling purposes.  Using the previously discussed logic, we can discard the remaining principle components and apply a varimax rotation to the retained PC1 and PC2 loadings to improve the interpretability of the results. We see that the weights of the factors within RC1 and RC2 are closer to 1 or 0. This is called the Varimax rotation. Note that the cumulative variance retained by R1 and R2 is the same as the rotated pca, but the sums of squares of both RC1 and RC2 more equal.
```{r}
...
ggplot(weights_varimax, aes(x = RC1, y = RC2))+geom_point()+
  scale_x_continuous(limits = c(-1,1)) +  # Define x-axis limits
  scale_y_continuous(limits = c(-1,1)) +  # Define y-axis limits
  geom_vline(xintercept = 0, color = "black", linetype = "dotted") +  # Vertical line for y-axis
  geom_hline(yintercept = 0, color = "black", linetype = "dotted") +  # Horizontal line for x-axis
  theme_minimal() + # Minimal theme for cleaner appearance
  geom_text(aes(label = rownames(weights_varimax)), vjust = 1.5, hjust = -0.5, size = 3)   # Text labels with row 

```

These new rotated dimensions capture the major axes of cockpit interaction. RC1 loads on torso measurements whereas R2 loads on limb measurements. By performing PCA on the reference population (e.g., CFAS), we identify the principal axes along which body proportions vary, reducing redundancy and emphasizing body shape over raw size. 

Each participant’s anthropometric profile is then projected into this retated PCA space, allowing us to plot them relative to the broader population distribution. This enables:
...
ggplot(pred_frame, aes(x = predicted, y = actual)) +
  ...
  annotate("text", x = Inf, y = -12, label = "Accommodated", hjust = 1.1, vjust = 1.5, color = "blue")+
...
df_summary <- df_new %>%
  mutate(predicted_class = ifelse(y_predicted > -12, "Dissaccommodated", "Accommodated")) %>%
...
# Rudder position model

model_rudder <- lm(rudder_mm ~ `Buttock-Knee Length` +
              `Combo Leg` + `Knee Height - Sitting`, data = df)
step_model_rudder <- stepAIC(model_rudder, direction = "both")
summary(step_model_rudder)
df_new$rudder_mm <- predict(step_model_rudder, newdata = df_new)
...
A linear regression model was created to predict aft clearance using three body measurements: Waist Depth, BMI, and Weight. Through model selection, Weight was found to have little added value and was removed. The final model included Waist Depth and BMI as predictors. We also plot predicted vs. actual clearance against an arbitrary functional requirement criterion. Finally, we use the model to predict the accommodation level of males and female target populations.

Although neither predictor reached individual statistical significance, together they explained a substantial portion of the variance in clearance (Adjusted R² = 0.918). The model was statistically significant overall (p < 0.001), suggesting that body size, particularly central mass indicators, is a strong predictor of cyclic-to-abdomen clearance.

The results highlight that bulkier body types (higher waist depth or BMI) may lead to reduced clearance, which could have implications for cockpit fit and safety.
...
model <- lm(aft_margin_body_mm ~ `Waist Depth` +
              BMI + Weight, data = df)
step_model <- stepAIC(model, direction = "both")
summary(step_model)

# Generate predicted values and confidence intervals
df$aft_margin_body_mm_predicted <- predict(step_model)
pred_frame <- data.frame(
  predicted = df$aft_margin_body_mm_predicted,
  actual = df$aft_margin_body_mm,
  ID = df$ID
)
...
ggplot(pred_frame, aes(x = predicted, y = actual)) +
  ...
  geom_hline(yintercept = 75, linetype = "dashed", color = "red") + # ONV requirement (arbitrary -12 degree requirement)+
  annotate("text", x = Inf, y = 75, label = "Accommodated", hjust = 1.1, vjust = -0.5, color = "blue")+
  annotate("text", x = Inf, y = 75, label = "Disccommodated", hjust = 1.1, vjust = 1.5, color = "red")+
...
df_summary <- df_new %>%
  mutate(predicted_class = ifelse(y_predicted < 75, "Dissaccommodated", "Accommodated")) %>%
  group_by(Sex, predicted_class) %>%
  count() %>%
  group_by(Sex) %>%
  mutate(percentage = round(100 * n / sum(n), 1)) %>%
...
kable(df_summary, caption = "Accommodation of Cyclic Control Motion - Zone 1 (75% fwd)")
...
df_summary <- df_new %>%
  mutate(predicted_class = ifelse(y_predicted < 0, "Dissaccommodated", "Accommodated")) %>%
  group_by(Sex, predicted_class) %>%
  count() %>%
  group_by(Sex) %>%
  mutate(percentage = round(100 * n / sum(n), 1)) %>%
...
df_expanded$y_predicted <- predict(model, newdata = df_expanded)

df_summary <- df_expanded %>%
  mutate(predicted_class = ifelse(y_predicted < 14, "Dissaccommodated", "Accommodated")) %>%
  group_by(Sex, predicted_class) %>%
  count() %>%
  group_by(Sex) %>%
  mutate(percentage = round(100 * n / sum(n), 1)) %>%
...
df_summary <- df_expanded %>%
  group_by(ID, Sex) %>%  
  summarise(
    any_accommodated = any(y_predicted > 13), # functional requirement
    .groups = "drop"
  ) %>%
  mutate(
